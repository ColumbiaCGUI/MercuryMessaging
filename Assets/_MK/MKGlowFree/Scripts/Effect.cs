    ...
    internal sealed class Effect
    {
        ...
        internal void Build(RenderTexture source, RenderTexture destination, Settings settings, Camera renderingCamera)
        {
            ...
            if(_settings.workflow == Workflow.Selective)
            {
                ...
                _selectiveRenderTarget = RenderTexture.GetTemporary(_renderingCamera.pixelWidth / 2, _renderingCamera.pixelHeight / 2, 16, _renderTextureFormat, RenderTextureReadWrite.Default, 1);//PipelineExtensions.GetTemporary(_selectiveRenderContext, _renderTextureFormat);
