...
The unified API provides **identical methods** for both `MmRelayNode` and `MmBaseResponder`, making it easy to send messages from any context:
...
These older methods still work but the unified API above is recommended:

```csharp
// Broadcast methods (use BroadcastInitialize(), BroadcastRefresh() instead)
...
// Notify methods (use NotifyComplete(), NotifyValue() instead)
