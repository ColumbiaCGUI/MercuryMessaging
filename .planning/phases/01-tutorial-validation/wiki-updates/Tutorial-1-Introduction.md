...
> **Tutorial Scene:** The actual script is `T1_ChildResponder.cs` (with `T1_` prefix per tutorial naming convention).
...
> **Tutorial Scene:** The actual script is `T1_TraditionalApiExample.cs`. It uses Space for MessageString and S for full metadata send.
...
> **Tutorial Scene:** The actual script is `T1_ParentController.cs`. It uses I for BroadcastInitialize, Alpha1 for Send string, Alpha2 for Send int, R for BroadcastRefresh, and Alpha3 for BroadcastValue(42).
...
public class ParentController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Alpha3))
            relay.BroadcastValue(42);  // Auto-detects int type
...
> **Tutorial Scene:** The actual script is `T1_ParentController.cs` (with `T1_` prefix).
...
> **Tutorial Scene:** The actual script is `T1_ChildResponder.cs` (with `T1_` prefix).

**Expected Output (on Play, then press 1, 2, 3, R):**
```
[T1_Child] Initialized!
[T1_Child] Received string: Message from parent!
[Parent] Sent string via .Send().ToChildren().Execute()
[T1_Child] Received int: 100
[Parent] Sent int via .Send().ToChildren().Execute()
[T1_Child] Received int: 42
[Parent] Sent BroadcastValue(42) - auto-detects type
[T1_Child] Refreshed
[Parent] Sent BroadcastRefresh()
```
...
The tutorial scene (`Tutorial1/`) includes additional scripts for a complete interactive demo:

| Key | Script | Action |
|-----|--------|--------|
| **I** | T1_ParentController | `BroadcastInitialize()` - Initialize all children |
| **1** | T1_ParentController | `Send("Message from parent!").ToChildren().Execute()` - String message |
| **2** | T1_ParentController | `Send(100).ToChildren().Execute()` - Int message |
| **3** | T1_ParentController | `BroadcastValue(42)` - Auto-typed broadcast |
| **R** | T1_ParentController | `BroadcastRefresh()` - Refresh all children |
| **Space** | T1_SceneController | `SetActive` toggle (traditional API) |
| **Space** | T1_TraditionalApiExample | `MmInvoke(MmMethod.MessageString, ...)` |
| **S** | T1_TraditionalApiExample | Full metadata send (traditional API) |

> **Note:** `T1_SceneController` toggles child active state using Space, demonstrating the traditional SetActive pattern. `T1_TraditionalApiExample` shows the verbose API for comparison with the DSL.
