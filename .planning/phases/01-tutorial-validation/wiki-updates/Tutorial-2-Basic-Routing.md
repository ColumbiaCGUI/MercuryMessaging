...
> **Tutorial Scene:** The actual script is `T2_RoutingExamples.cs` (with `T2_` prefix per tutorial naming convention).
...
The tutorial scene (`Tutorial2/`) uses `T2_RoutingExamples.cs` with these keyboard shortcuts:

| Key | Direction Method | Description |
|-----|-----------------|-------------|
| **C** | `.ToChildren()` | Send to direct children only |
| **P** | `.ToParents()` | Send to direct parents only |
| **D** | `.ToDescendants()` | Send to all descendants recursively |
| **A** | `.ToAncestors()` | Send to all ancestors recursively |
| **S** | `.ToSiblings()` | Send to same-level nodes |
| **F** | `.ToAll()` | Send in all directions (bidirectional) |

**Expected Console Output (press C):**
```
[Routing] Sent .ToChildren() - direct children only
  [Child1] Received string: "Hello children"
  [Child2] Received string: "Hello children"
```
...
> **Tutorial Scene:** The actual script is `T2_MenuController.cs` (with `T2_` prefix).
...
> **Tutorial Scene:** The actual script is `T2_ButtonResponder.cs` (with `T2_` prefix).
