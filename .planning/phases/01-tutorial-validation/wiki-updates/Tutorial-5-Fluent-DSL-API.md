...
```
  [Child1] Initialize() called
  [Child2] Initialize() called
  [Grandchild1] Initialize() called
```
...
The framework also provides DSL temporal methods for delayed and repeating messages:

```csharp
// Delayed execution (executes after 2 seconds)
relay.After(2f, MmMethod.Initialize);

// Repeating messages (every 1 second, 5 times)
relay.Every(1f, MmMethod.Refresh, repeatCount: 5);

// Conditional execution (fires when condition becomes true)
relay.When(() => isReady, MmMethod.Initialize);
```

> **Note:** The tutorial demo scene uses coroutines as a manual alternative for the delayed and repeating demos. Both approaches (DSL temporal methods and coroutines) are valid.
...
**Location:** `Assets/MercuryMessaging/Examples/Tutorials/Tutorial5/`

### Setup

1. Open the Tutorial5 scene in Unity Editor
2. Press Play
3. See console for output instructions

> **Tutorial Scene:** The main script is `T5_DSLSceneSetup.cs`. It creates the hierarchy programmatically and demonstrates all DSL features.
