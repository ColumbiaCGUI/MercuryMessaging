...
| D-10 | **Fluent DSL for message routing** (existing) | Two-tier API matching ceremony to routing complexity: property routing (`relay.To.Children.Send(...)`) for concise fire-and-forget, fluent chains (`relay.Send(...).ToDescendants().Within(10f).Execute()`) for composable spatial filtering. Discoverability via IDE autocompletion at each step. No competing framework offers this level of API ergonomics for message routing in C#/Unity. Note: original API also supports default parameters for simple invocations (`relay.MmInvoke(MmMethod.X, value)`); the DSL's advantage is discoverability and spatial filter composability, not raw line-count reduction. | Already implemented | N/A (core framework) | Already exists. Strong contribution point for the paper's "ease of combination" (Olsen criterion). Paper frames advantage as discoverability + composable spatial filtering, not line-count reduction. |
